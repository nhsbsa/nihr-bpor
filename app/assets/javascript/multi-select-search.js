(function () {
  ...
  var modules = document.querySelectorAll(
    '[data-module="searchable-checkboxes"]'
  );
  ...
  modules.forEach(function (container) {
     ...
     *
     * showAllOptions:
     * false = options remain hidden until the user searches
     * true  = options are visible immediately and can be scrolled
     */
    var showAllOptions =
      container.getAttribute('data-show-all-options') === 'true';
    ...
    searchInput.addEventListener('input', function () {
      ...
      /*
       * When showAllOptions is enabled, the list remains
       * visible regardless of whether there is a search term.
       *
       * When disabled, the existing behaviour is retained:
       * the list only appears once the user starts typing.
       */
      if (showAllOptions || term.length > 0) {
    ...
    function renderTags() {
      ...
      checked.forEach(function (checkbox) {
        ...
        var labelEl = container.querySelector(
        ...
        li.className =
          'nhsuk-tag nhsuk-tag--blue nhsuk-tag--filter nhsuk-u-margin-right-2';

        var text = document.createElement('span');

        text.className = 'nhsuk-tag__text';

        text.textContent = labelEl.textContent.trim();
        ...
        btn.setAttribute(
          'aria-label',
          'Remove ' + labelEl.textContent.trim()
        );
    ...
    /*
     * If showAllOptions is enabled, display the
     * checkbox list immediately.
     */
    if (showAllOptions) {
      listEl.classList.add('is-visible');
    }
